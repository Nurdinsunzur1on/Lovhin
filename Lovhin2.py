...
owner = Din_Lovhin.LINE() #Akun Utama
owner.login(token="token")
owner.loginResult()
    
Lovhin = Din_Lovhin.LINE()
...
ki10 = Din_Lovhin.LINE() #Ghost
ki10.login(token="token")
ki10.loginResult()
print "═══「 Din Lovhin login success」═══"
reload(sys)
sys.setdefaultencoding('utf-8')
helpMenu="""╔═══「 Din Lovhin ʙᴏᴛ」═══╗
╠╦═══════════════
║║🔰 Me
║║🔰 Add
║║🔰 Gift
...
creator=["u097bef0ac47864896f2bbc5ff7721193"]
admsa=["u097bef0ac47864896f2bbc5ff7721193"]

readOpen = codecs.open("st2__b.json","r","utf-8")
read = json.load(readOpen)

contact = Lovhin.getProfile()
restoreprofile = Lovhin.getProfile()
...
wait = {
    ...
    'sider1':"CCTV Ngintip mulu,, chat gx jika gx chat saya makan nih 😂😂😂",
    'joingc':"WELCOME",
    'leftgc':"Papay... 😢😢😢",
...
def bot(op):
    try:
        ...
        if op.type == 13:
            if mid in op.param3:
                G = lovhin.getGroup(op.param1)
                if wait["autoJoin"] == True:
                    if wait["autoCancel"]["on"] == True:
                        if len(G.members) <= wait["autoCancel"]["members"]:
                            vipro.rejectGroupInvitation(op.param1)
                        else:
                            lovhin.acceptGroupInvitation(op.param1)
			    G.preventJoinByTicket = False
			    vipro.updateGroup(G)
			    Ticket = lovhin.reissueGroupTicket(op.param1)
			    ...
			    lovhin.updateGroup(G)
                    else:
                        lovhin.acceptGroupInvitation(op.param1)
                elif wait["autoCancel"]["on"] == True:
                    if len(G.members) <= wait["autoCancel"]["members"]:
                        lovhin.rejectGroupInvitation(op.param1)
            else:
              ...
              if not op.param2 in admsa and creator:
                if wait["protect"] == True: 
                    try:
                        ...
                        lovhin.inviteIntoGroup(op.param1,[op.param3])
                    except Exception, e:
                       print e                                       
        ...
        if op.type == 13:
            if op.param3 in mid:
                if op.param2 in kimid:
                    ...
                    lovhin.acceptGroupInvitationByTicket(op.param1,Ticket)
                    G.preventJoinByTicket = True
                    kimid.updateGroup(G)
                    Ticket = kimid.reissueGroupTicket(op.param1)
            ...
            if op.param3 in kimid:
                if op.param2 in mid:
                    X = lovhin.getGroup(op.param1)
                    X.preventJoinByTicket = False
                    lovhin.updateGroup(X)
                    Ti = lovhin.reissueGroupTicket(op.param1)
        ...
        if op.type == 19:
            if mid in op.param3:
             ...
             #     if wait["Ghost"] == True:  
              #     try:
               #        klist=[lovhin,ki,ki2,ki3,ki4,ki5]
                #       kicker = random.choice(klist)
                 #      G = kicker.getGroup(op.param1)
                  #     G.preventJoinByTicket = False
        ...
        if op.type == 26:
                ...
                if msg.from_ == mid:
                    if "join:" in msg.text:
                        ...
                        try:
                            ...
                            G = lovhin.getGroup(list_[1])
                            G.preventJoinByTicket = True
                            vipro.updateGroup(G)
                        except:
                            lovhin.sendText(msg.to,"error")
            if msg.toType == 1:
                if wait["leaveRoom"] == True:
                    lovhin.leaveRoom(msg.to)
            ...
            if msg.contentType == 16:
              if wait["likeOn"] == True:
                ...
                lovhin.like(url[25:58], url[66:], likeType=1001)
                ...
                lovhin.comment(url[25:58], url[66:], wait["comment1"])
...
#-----------------------------------------------
        ...
        if op.type == 26:
                    ...
                    if msg.text is not None:
                        ...
                        if data['status'] == 200:
                            if data['result']['result'] == 100:
                                lovhin.sendText(msg.to, "🤠" + data['result']['response'].encode('utf-8'))
            if "MENTION" in msg.contentMetadata.keys() != None:
                 if wait['detectMention'] == True:
                     contact = lovhin.getContact(msg.from_)
                     ...
                     for mention in mentionees:
                           if mention["M"] in mid:
                                  ...
                                  lovhin.sendMessage(msg)
                                  lovhin.sendText(msg.to, wait["tagme"])
                                  break
            if "MENTION" in msg.contentMetadata.keys() != None:
                ...
                if wait['invite'] == True:
                     ...
                     for s in groups.members:
                         if _name in s.displayName:
                             lovhin.sendText(msg.to, _name + " Berada DiGrup Ini")
                         else:
                     ...
                     if targets == []:
                         ...
                         for target in targets:
                             try:
                                 lovhin.findAndAddContactsByMid(target)
                                 lovhin.inviteIntoGroup(msg.to,[target])
                                 lovhin.sendText(msg.to,"Invite " + _name)
                                 wait['invite'] = False
                                 break                              
                             except:             
                                      lovhin.sendText(msg.to,"Error")
            ...
            if wait["alwayRead"] == True:
                if msg.toType == 0:
                    lovhin.sendChatChecked(msg.from_,msg.id)
                else:
                    lovhin.sendChatChecked(msg.to,msg.id)
            if wait["Removechat"] == True:
                if msg.toType == 0:
                    lovhin.removeAllMessages(op.param2)
                else:
                    lovhin.removeAllMessages(op.param2)
        if op.type == 26:
            msg = op.message
            if msg.contentType == 13:
                if wait["wblack"] == True:
                    if msg.contentMetadata["mid"] in wait["commentBlack"]:
                        lovhin.sendText(msg.to,"sudah masuk daftar hitam👈")
                        wait["wblack"] = False
                    else:
                        wait["commentBlack"][msg.contentMetadata["mid"]] = True
                        wait["wblack"] = False
                        lovhin.sendText(msg.to,"Itu tidak berkomentar👈")
                elif wait["dblack"] == True:
                    if msg.contentMetadata["mid"] in wait["commentBlack"]:
                        del wait["commentBlack"][msg.contentMetadata["mid"]]
                        lovhin.sendText(msg.to,"Done")
                        wait["dblack"] = False
                    else:
                        wait["dblack"] = False
                        lovhin.sendText(msg.to,"Tidak ada dalam daftar hitam👈")
                    ...
                    msg.contentType = 0
                    lovhin.sendText(msg.to,msg.contentMetadata["mid"])
                    if 'displayName' in msg.contentMetadata:
                        contact = lovhin.getContact(msg.contentMetadata["mid"])
                        try:
                            cu = lovhin.channel.getCover(msg.contentMetadata["mid"])
                        except:
                            cu = ""
                        lovhinsendText(msg.to,"[displayName]:\n" + msg.contentMetadata["displayName"] + "\n[mid]:\n" + msg.contentMetadata["mid"] + "\n[statusMessage]:\n" + contact.statusMessage + "\n[pictureStatus]:\nhttp://dl.profile.line-cdn.net/" + contact.pictureStatus + "\n[coverURL]:\n" + str(cu))
                    else:
                        contact = lovhin.getContact(msg.contentMetadata["mid"])
                        try:
                            cu = lovhin.channel.getCover(msg.contentMetadata["mid"])
                            ...
                            cu = ""
                        try:
                            lovhin.sendImageWithURL(msg.to, "http://dl.profile.line-cdn.net/" + contact.pictureStatus)
                        except:
                            cu = ""
                        lovhin.sendText(msg.to,"🎀═displayName═🎀\n✤[" + contact.displayName + "]✤\n🎀═MIDs═🎀\n✤[" + msg.contentMetadata["mid"] + "]✤\n🎀═StatusContact═🎀\n✤" + contact.statusMessage + "✤")
                        lovhin.sendText(msg.to,"LINKPROFILE\nhttp://dl.profile.line-cdn.net/" + contact.pictureStatus + "\nLINKBERANDA\n" + str(cu))
                ...
                if wait["timeline"] == True:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        msg.text = "URLâ†’\n" + msg.contentMetadata["postEndUrl"]
                    lovhin.sendText(msg.to,msg.text)
              ...
              if msg.from_ in creator:    
                ...
                if wait["lang"] == "JP":
                  ...
                  lovhin.sendText(msg.to,"He declined all invitations")
#--------------------------
            elif msg.text in ["Leaveallgroup"]: 
              if msg.from_ in creator:
                gid = lovhin.getGroupIdsJoined()
                for i in gid:
                  lovhin.leaveGroup(i)
                if wait["lang"] == "JP":
                  lovhin.sendText(msg.to,"Bye~Bye " + str(ginfo.name) + "\n\nBots Dipaksa Keluar oleh selfbot...!!!\nMakasih...!!!")
                else:
                  lovhin.sendText(msg.to,"He declined all invitations")
#----------------------------------------------
              ...
              if msg.from_ in creator + admin:
                    ...
                    n = lovhin.getGroupIdsJoined()
                    for manusia in n:
                        lovhin.sendText(manusia, (bctxt))
            elif "/Sendcontact " in msg.text:
              if msg.from_ in creator + admin:
					bctxt = msg.text.replace("/Sendcontact ", "")
					t = lovhin.getAllContactIds()
              ...
              if Bots == []:
                  lovhin.sendText(msg.to,"The Friends is empty")
              else:
                  lovhin.sendText(msg.to,"Tunggu...")
                  mc = "||===FRIENDLIST===||\n=====================\n"
                  for mi_d in Bots:
                      mc += "★" +lovhin.getContact(mi_d).displayName + "\n"
                  lovhin.sendText(msg.to,mc)
                  print "[Command]Friendlist executed"                    
            elif "Youinfo" in msg.text:
              if msg.from_ in creator + admin:
                key = eval(msg.contentMetadata["MENTION"])
                key1 = key["MENTIONEES"][0]["M"]
                contact = lovhin.getContact(key1)
                cu = lovhin.channel.getCover(key1)
                path = str(cu)
                image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                try:
                    lovhin.sendText(msg.to,"Nama :\n" + contact.displayName + "\n\nBio :\n" + contact.statusMessage)
                    lovhin.sendText(msg.to,"Profile Picture " + contact.displayName)
                    lovhin.sendImageWithURL(msg.to,image)
                    lovhin.sendText(msg.to,"Cover " + contact.displayName)
                    lovhin.sendImageWithURL(msg.to,path)
                except:
                    pass
            elif "Botak" in msg.text:
              if msg.from_ in creator + admin:
                group = lovhin.getGroup(msg.to)
                ...
                for j in xrange(k+1):
                    ...
                    b = urllib.quote(a)
                    lovhin.sendText(msg.to,"「 Searching 」\n" "Type: GitHub Search\nStatus: Processing...")
                    lovhin.sendText(msg.to, "Title: " + a + "\nLink: https://github.com/search?utf8=✓&q="+b)
            elif 'playstore ' in msg.text.lower():
                    tob = msg.text.lower().replace('playstore ',"")
                    lovhin.sendText(msg.to,"Please wait...")
                    lovhin.sendText(msg.to,"Title : "+tob+"\nSource : Google Play\nLinknya : https://play.google.com/store/search?q=" + tob)
                    lovhin.sendText(msg.to,"This is link aplication")     
                  ...
                  try:
                      ...
                      lovhin.sendText(msg.to, pesan)
                          ...
                          try:
                              ...
                              lovhin.sendText(msg.to, str(e))
            elif "Twitter " in msg.text:
                    a = msg.text.replace("Twitter ","")
                    b = urllib.quote(a)
                    lovhin.sendText(msg.to,"「 Searching 」\n" "Type:Search Info\nStatus: Processing")
                    lovhin.sendText(msg.to, "https://www.twitter.com" + b)
                    lovhin.sendText(msg.to,"「 Searching 」\n" "Type:Search Info\nStatus: Success") 
            elif "Smule " in msg.text:
                    a = msg.text.replace("Smule ","")
                    b = urllib.quote(a)
                    lovhin.sendText(msg.to,"Searching to id smule..")
                    lovhin.sendText(msg.to, "Nama: "+b+"\nId smule: http://smule.com/" +b)
            elif "Google " in msg.text:
                    a = msg.text.replace("Google ","")
                    b = urllib.quote(a)
                    lovhin.sendText(msg.to,"Searching google..")
                    lovhin.sendText(msg.to, "Search: "+b+"\nsuccess: http://google.com/" +b)
	    elif "Xvideos " in msg.text:
		    a = msg.text.replace("Xvideos ","")
                    b = urllib.quote(a)
                    lovhin.sendText(msg.to,"Searching ....\n" "Type:Search Xvideos\nStatus: Processing")
                    lovhin.sendText(msg.to, "{ Xvideos search page }\n\nTitle: "+b+"\nSource : Xvideos\nhttp://xvideos.com/?k=" +b)
                ...
                if targets == []:
                    lovhin.sendText(msg.to,"Contact not found")
                else:
                    for target in targets:
                        try:
              ...
              if msg.from_ in creator + admin:
                ...
                if targets == []:
                    ...
                    for target in targets:
                        try:
                            contact = lovhin.getContact(target)
                            cu = lovhin.channel.getCover(target)
                            path = str(cu)
                            lovhin.sendText(msg.to, path)
                        except Exception as e:
                            raise e
                print "[Command]cover executed"         
            elif msg.text in ["Pmfavorite"]:
              if msg.from_ in creator + admin:
                dj = lovhin.getFavoriteMids()
                kontak = lovhin.getContacts(dj)
                num = 1
                family = str(len(dj))
                ...
                lovhin.sendText(msg.to, msgs)
            elif msg.text.lower()  == 'setauto':
              if msg.from_ in creator + admin:
                   lovhin.sendText(msg.to,helpFun)
            elif msg.text.lower() == 'help':
              if msg.from_ in creator + admin:
                if wait["lang"] == "JP":
                    lovhin.sendText(msg.to,helpMessage)
                else:
                    lovhin.sendText(msg.to,helpMessage)
            elif msg.text.lower() == 'menu':
              if msg.from_ in creator + admin:
                if wait["lang"] == "JP":
                    lovhin.sendText(msg.to,helpMenu)
                else:
                    lovhin.sendText(msg.to,helpMenu)
            elif msg.text.lower() == 'media':
              if msg.from_ in creator + admin:
                if wait["lang"] == "JP":
                    lovhin.sendText(msg.to,helpMedia)
                else:
                    lovhin.sendText(msg.to,helpMedia)
            elif msg.text.lower() == 'self':
              if msg.from_ in creator + admin:
                if wait["lang"] == "JP":
                    lovhin.sendText(msg.to,helpself)
                else:
                    lovhin.sendText(msg.to,helpself)
            elif msg.text.lower() == 'settings':
              if msg.from_ in creator + admin:
                if wait["lang"] == "JP":
                    lovhin.sendText(msg.to,helpset)
                else:
                    lovhin.sendText(msg.to,helpset)
            elif ("Gn:" in msg.text):
              if msg.from_ in creator + admin:
                if msg.toType == 2:
                    ...
                    lovhin.updateGroup(group)
                else:
                    lovhin.sendText(msg.to,"Hal ini tidak dapat digunakan di luar kelompok👈")
            elif ("Gn " in msg.text):
              if msg.from_ in creator + admin:
                if msg.toType == 2:
                    ...
                    lovhin.sendText(msg.to,"Can not be used for groups other than")
            elif "Kick:" in msg.text:
              if msg.from_ in creator + admin:
                midd = msg.text.replace("Kick:","")
                lovhin.kickoutFromGroup(msg.to,[midd])
            elif "Invite:" in msg.text:
              if msg.from_ in creator + admin:
                midd = msg.text.replace("Invite:","")
                lovhin.findAndAddContactsByMid(midd)
                lovhin.inviteIntoGroup(msg.to,[midd])
            elif "Me" == msg.text:
                       msg.contentType = 13
                       msg.contentMetadata = {'mid': msg.from_}
                       lovhin.sendMessage(msg)
              ...
              if msg.from_ in creator + admin:
                ...
                msg.contentMetadata = {'mid': ki9mid}
                ki9.sendMessage(msg)
                ki.sendText(msg.to,"AMAN TERKENDALI KAPTEN👮")
            elif "B1" == msg.text:
              if msg.from_ in creator + admin:
                msg.contentType = 13
                msg.contentMetadata = {'mid': kimid}
